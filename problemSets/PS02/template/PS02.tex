...
\begin{enumerate}
	...
	\item Make a bar plot with \texttt{geom\_col()} detailing the number of official members using the 12-level religious classification (\texttt{TRAD12}) distinguishing between the 6-level religious classification (\texttt{TRAD6}) in 2022. Hint: Use \texttt{facet()} for \texttt{TRAD6}, with \texttt{TRAD12} on the x-axis.
	\item Display the distribution of average yearly income (\texttt{INCOME}) for congregations in 2022 in each region (\texttt{GDREGION}) using ridge plots.
